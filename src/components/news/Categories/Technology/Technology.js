...
class Technology extends Component {
    state = {
        news: [],
        heading: 'Top Headlines In Technology',
        id: uuid()
    }

    componentDidMount() {
        axios.get('https://newsapi.org/v2/top-headlines?category=technology&pageSize=24&apiKey=0fb4e3906e844d25a06d3bcb65fceb49')
            .then(res => {
                    ...
                    <div className="row">
                        {news.map(article => (
                            <div className="col-md-6 mb-4">
                                <TechnologyNews article={article} key={id}/>
                            </div>
                        ))}
                    </div>
                </div>
